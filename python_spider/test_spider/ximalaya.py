...
class Radio_spider():
    ...
    def __init__(self):
        ...
        self.url_temp = "http://www.ximalaya.com/96373069/album/11709082/"
        self.headers = {
    ...
    def parse_url(self):
        ...
        dict = resp.text
        pattern = re.compile(r'href="/\d+/sound/(\d+)/')
    ...
    def get_content(self, url, name):
        radio = requests.get(url, headers=self.headers)
        file_path = name + '.m4a'
    ...
    def run(self):
        ...
        url_list = []
        for id in id_list:
            url = "http://www.ximalaya.com/tracks/"+str(id)+".json"
            url_list.append(url)
