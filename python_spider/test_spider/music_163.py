...
class Music163:
    ...
    def get_playlist_list(self, item, total_playlist_list):  # 获取小分类中的playlist列表
        ...
        if item["s_href"] is not None:
            ...
            for li in li_list:
                ...
                item["author_name"] = li.xpath("./p[last()]/a/@title")[0] if len(
                    li.xpath("./p[last()]/a/@title")) > 0 else None
                item["author_href"] = "http://music.163.com" + li.xpath("./p[last()]/a/@href")[0] if len(
                    li.xpath("./p[last()]/a/@href")) > 0 else None
    ...
    def get_playlist_info(self, playlist):  # 获取单个播放别表的信息
        if playlist["playlist_href"] is not None:
            ...
            playlist["favorited_times"] = playlist_html.xpath("//a[@data-res-action='fav']/@data-count")[0] if len(
                playlist_html.xpath("//a[@data-res-action='fav']/@data-count")) > 0 else None
            playlist["shared_times"] = playlist_html.xpath("//a[@data-res-action='share']/@data-count")[0] if len(
            ...
            playlist["played_times"] = playlist_html.xpath("//strong[@id='play-count']/text()")[0] if len(
